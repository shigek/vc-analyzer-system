...
export class ResolverController {
  constructor(private shareService: ShareService, private readonly resolverService: ResolverService) { }
  ...
  async getDidDocument(@Headers('X-Correlation-ID') correlationId: string, @Param('did') did: string, @Res() res: Response): Promise<any> {
    ...
    try {
      ...
      const didDocument = await this.resolverService.getDidDocument(did, newCorrelationId);
